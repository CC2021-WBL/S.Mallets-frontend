...
import { navActions } from './languageSlice';
  ...
  return (
    <div className="flex justify-between items-center h-20 sm:h-36 md:h-44 max-w-7xl mx-auto sm:px-3 md:px-6 lg:px-8">
      <div
        className={
          !showNav
            ? 'p-6 sm:p-3 lg:p-4 w-36 sm:w-48  md:w-64 lg:w-72'
            : 'hidden'
        }
      >
      ...
      >
        ...
        <nav className="tracking-wider p-4 flex flex-col z-50">
          <NavBar
            t={t}
            className={navBurgerStyles}
            handleNav={toggleNavHandler}
          />
        </nav>
      ...
      {showNav && <Backdrop handleNav={toggleNavHandler} />}
    </div>
  );
};
